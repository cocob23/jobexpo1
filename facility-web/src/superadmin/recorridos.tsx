...
export default function RecorridosAdmin() {
  ...
  // Buscar sugerencias de empleados (roles mantenimiento/mantenimiento_externo) case-insensitive
  useEffect(() => {
    ...
    const handler = setTimeout(async () => {
      ...
      const { data, error } = await supabase
        ...
        .limit(10)
      if (!error && data) {
        const nombres = data.map(u => (u.nombre || '') + (u.apellido ? ` ${u.apellido}` : '') || u.email).filter(Boolean) as string[]
  ...
  const cargar = async (aplicarFiltros = false) => {
    ...
    if (aplicarFiltros) {
      if (fechaDesde.trim()) {
        query = query.gte('creado_en', fechaDesde + ' 00:00:00')
      }
      if (fechaHasta.trim()) {
        query = query.lte('creado_en', fechaHasta + ' 23:59:59')
      }
    }
    ...
    if (!error && data) {
      ...
      // Mostrar todos (si querés volver a roles, reintroducir filtro aquí)
              ...
              {sugerencias.length > 0 ? (
                sugerencias.map(s => (
                  <div
                    ...
                    onMouseDown={() => { // onMouseDown evita blur antes de seleccionar
                      setFiltroNombre(s)
                      setMostrandoSug(false)
                    }}
                  >{s}</div>
                ))
              ) : (

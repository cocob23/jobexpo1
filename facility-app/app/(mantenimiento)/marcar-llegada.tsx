...
export default function MarcarLlegadaMantenimientoScreen() {
  ...
  const [llegadasRecientes, setLlegadasRecientes] = useState<any[]>([])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (error) {
        ...
        Alert.alert(
          'Permisos',
          'No se pueden leer las empresas. Avisá al admin para habilitar SELECT a rol "mantenimiento".'
        )
      } else {
        setEmpresas(data || [])
      }
    ...
    // Cargar llegadas recientes del usuario
    ;(async () => {
  ...
  const marcarLlegada = async () => {
    ...
    if (error) {
      ...
      setFiltradas([])
      // Actualizamos el historial reciente
      const { data: userData } = await supabase.auth.getUser()
      const userId = userData?.user?.id
      if (userId) {
        const { data } = await supabase
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      ...
      {llegadasRecientes.length > 0 && (
        <View style={{ marginTop: 24 }}>
          <Text style={styles.subtitulo}>Tus llegadas recientes</Text>
          {llegadasRecientes.map((l) => (
            <View key={l.id} style={styles.itemLlegada}>
              <Text style={{ fontWeight: '600' }}>{l.lugar}</Text>
              <Text style={{ color: '#374151' }}>📅 {l.fecha}  ·  🕒 {l.hora}</Text>
            </View>
          ))}
        </View>
      )}
